...
- **跨章状态统一交接与回放校验策略（规则 2.10.0）**：远征委托 / 奇遇印记 /
  药水背包 / 伙伴 / 协作队伍归属五类「随交接快照跨章继承」的 run 状态维度，
  统一由 `app/chapterstate.py` 的**维度注册表**驱动——新档默认值、交接快照
  取值（`carry_snapshot`）、旧存档迁移（`migrate_run_state`）、回放旧 create
  校验点形状候选（四维 16 种；委托从远征上线起就在哈希中、不剥维）与逐动作的
  「迁移点规则先行切换 + 旧区间 legacy 标记」（`LegacyWindows`：结构维与
  2.7.0 纯格挡规则窗口统一跟踪）全部从同一张注册表派生，新增跨章维度只需在
  一处登记、六处集成点自动对齐。失败回滚（纯推演异常整体回滚）、`request_id`
  幂等、`rev` 乐观锁、多人权限边界与每步 `actor` 沿用既有统一事务/动作日志；
  续局、章节/远征结算与整程回放逐位一致，旧档迁移与 2.1.0 以来各历史形状逐位
  兼容（状态形状与校验点相对 2.9.0 不变，本次为内部策略重构）。
...
携带 member_id 行为不变）**、
**跨章状态统一交接与回放校验策略 2.10.0（注册表单点登记五类跨章维度的
key/起始版本/fresh 默认/旧档 normalize/附属字段；fresh 与 carry 取值对称且
深拷贝隔离；缺字段旧快照按维度兜底；migrate_run_state 幂等补齐全部维度并
修复损坏伙伴/enc_state、在线续局后可行动；形状矩阵恰为四维 16 种且 full 优先、
顺序与历史嵌套逐位一致、commissions 恒在哈希；LegacyWindows 在迁移动作之前
统一切换结构维与纯格挡规则窗口、close_all 一次性越过、step_pre_version 以
本步录制版本判定旧规则产物；carry_snapshot 往返 _new_run_state 后印记开章
兑现/续写 flag 保留/新 run 身份只认真实入参；合法两章单人流程五类状态随同一
快照交接、整章/整程回放零 mismatch 且第 2 章首帧与在线逐位一致；协作远征战斗位
/资源位分别操作同一条日志、越权 403 零副作用、推进后跨章、整程回放零 mismatch
且每步操作者与 ledger 正确）**。
