...
- **跨章状态统一策略（2.9.1，`app/crosschapter.py`）**：远征委托、奇遇印记、药水背包、
  伙伴、协作队伍归属等跨章状态由**同一个数据驱动注册表** `FIELDS` 统一编排，每维只在一处
  声明字段名 / 起始规则版本 / 默认值 / 旧档规范化 / 旧校验点形状标记 / 回放结构穿越钩子，
  五条此前各自维护的链路全部从注册表取字段，新增维度不再需要五处同改、杜绝漏维：
  ① **交接**——章节通关 `extract_carry` 统一提取（深拷贝、旧损坏快照 NULL 值归一），
  开新章 `value_for_new_run` 统一重建，在线推进与回放重建共用 `_new_run_state` 同一路径；
  ② **旧存档迁移**——首次续局/首行动 `migrate_run_state` 幂等补齐各维，结构变化随本步动作
  同事务落库，推演失败整体回滚；③ **回放校验形状**——旧 create 校验点按 2^N 种字段形状
  候选（注册表维序驱动）匹配，缺维旧档逐位可比；④ **规则先行穿越**——首个 ≥ 该维起始版本
  的动作之前先完成结构切换（补字段/剥旧货架/清瞬态标记），再推演动作，与在线迁移严格对齐；
  ⑤ **多人操作**——协作队伍归属（`coop_team`）本身就是注册维，随交接快照跨章，权限边界
  在任何状态变更前校验（越权 403、零副作用），每步日志带 `actor`。续局、章节结算与
  整程回放因此始终逐位一致（规则重构、无规则/存档语义变更，历史校验点哈希逐位保持）
...
携带 member_id 行为不变）**、
**跨章状态统一策略 2.9.1（注册表 FIELDS 四维键/版本/marker 唯一、交接快照覆盖全部注册维
且深拷贝解耦、carry→新章 round-trip、在线推进与回放重建同校验点、旧档缺维幂等迁移、
损坏同伴/印记状态规范化、缺维旧档续局后回放零 mismatch、2^N 种旧 create 形状候选逐位匹配、
损坏 carry 的 NULL 值在交接/公开视口兜底、协作队伍 coop_team 随交接跨章且越权 403 零副作用）**。
...
- **跨章状态注册表（`app/crosschapter.py`，规则 2.9.1）**：药水背包（2.5）/伙伴（2.6）/
  奇遇印记 enc_state（2.8）/协作队伍 coop_team（2.9）四个结构维在 `FIELDS` 中各声明一次，
  交接快照提取（`extract_carry`）、新章重建（`value_for_new_run`，在线与回放共用
  `_new_run_state`）、旧档迁移（`migrate_run_state`，幂等、随行动事务原子落库）、
  `state_checkpoint` 的 2^N 旧形状候选（`checkpoint_kwargs`/`shape_name`）与回放的
  「规则先行」结构穿越（`enter_legacy`/`cross`/`strip_shop_shelf`：首个该维版本动作之前
  补字段、剥旧货架、清瞬态 marker）全部由注册表统一驱动；旧损坏 carry 中的 NULL 值在
  交接/公开视口边界按各维默认值兜底。维序即规则演进顺序（有语义：决定候选命名与后缀关系），
  纯重构不改任何规则结果，历史 create/动作校验点逐位不变，全套旧版回放测试保持通过。
