...
from . import chapterstate as cs
...
# 2.10.0：跨章状态统一交接与回放校验策略——远征委托 / 奇遇印记 / 药水背包 /
#        伙伴 / 协作队伍归属五类「随交接快照跨章继承」的 run 状态维度改由
#        app/chapterstate.py 的注册表统一描述：新档默认值、交接快照取值、
#        旧存档迁移、回放旧 create 形状候选（结构维组合矩阵）、逐动作的
#        「迁移点规则先行切换 / 旧区间 legacy 标记」全部从注册表派生，
#        新增跨章维度只需在一处登记。本版为内部策略重构：状态形状、校验点
#        与既有规则语义逐位不变（新档事件打 2.10.0 版本标签，回放旧日志的
#        兼容窗口仍按各维度自身版本切换）。
...
def _new_run_state(seed, carry=None, chapter=None, chapters_total=None, expedition_id=None,
    ...
    if carry is None:
        ...
        # 跨章维度默认值全部由注册表给出（药水/伙伴/奇遇/委托/协作归属）
        dims = cs.fresh_dimension_values()
        carry = {
    ...
    # 跨章维度统一从交接快照取值（缺字段按各维度兜底），再跑入章结算
    dims = cs.carry_dimension_values(carry)
    aux = cs.carry_auxiliary_values(carry)
    chapter_for_enc = chapter if chapter is not None else carry.get("chapter")
    opener_heal = cs.enter_new_chapter(dims, chapter_for_enc)
    ...
    return {
        ...
        # 药水背包 / 伙伴 / 奇遇印记 / 远征委托 / 协作归属：注册表统一交接
        **dims,
        # 远征委托 uid 单调发号器（commissions 维度附属字段）
        "next_commission_seq": aux["next_commission_seq"],
        # 协作队伍归属：显式入参优先（协作开章/推进带 team id），否则从快照继承
        "coop_team": (coop_team if coop_team is not None else dims["coop_team"]),
...
def _migrate_state(run):
    ...
    # 跨章状态维度（2.2.0 委托 / 2.5.0 药水 / 2.6.0 伙伴 / 2.8.0 奇遇 /
    # 2.9.0 协作归属）：缺字段补默认、有字段幂等规范化，全部由注册表驱动
    if cs.migrate_run_state(run):
        changed = True
    # 远征身份列不属于跨章维度（不随交接快照决定新 run 身份），单独兜底
...
def _carry_from_run(run):
    ...
    牌组（含锻造成长）、遗物、金币、生命与能量上限等核心字段，加上注册表
    登记的全部跨章维度（远征委托含进度/可领奖/已终结状态与 uid 发号器、
    药水背包、伙伴、奇遇印记、协作队伍归属），字段清单以 chapterstate 注册表
    为唯一权威。
    """
    return cs.carry_snapshot(run)
...
# 旧区间抑制标记 / 纯规则窗口标记键由 chapterstate 注册表统一给出
# （SUPPRESS_KEYS、RULE_WINDOWS）；这里保留模块级别名供动作推演引用。
_LEGACY_NO_COMPANION_KEY = "_legacy_no_companion"
_LEGACY_NO_POTIONS_KEY = "_legacy_no_potions"
_LEGACY_BLOCK_KEY = "_legacy_block"
# 回放旧规则窗口的瞬态标记键（不写入存档、不参与校验点哈希）由注册表给出：
# 结构维度的旧规则抑制标记 + 纯规则窗口标记（如 2.7.0 旧格挡时序）。
_CKPT_SKIP_KEYS = {
    "events_log", _PENDING_UNLOCK_KEY, "_ckpt_skip_keys",
    *cs.SUPPRESS_KEYS.values(),
    *(key for key, _ver in cs.RULE_WINDOWS),
}


def state_checkpoint(run, include_dimensions=None, *,
                     include_companion=None, include_potions=None,
                     include_encounters=None, include_coop=None):
    """权威状态校验点：对完整 run 状态取稳定哈希（SHA-256 截断 16 位）。

    include_dimensions 非 None 时为 {维度key: 是否参与哈希}（注册表形状矩阵
    使用）；历史调用方的 include_potions/include_companion/include_encounters/
    include_coop 关键字参数仍兼容（只剥对应单维，其余维度按完整形状参与）。
    缺省为完整形状——参与哈希的维度集合由注册表统一给出。
    """
    skip = set(_CKPT_SKIP_KEYS)
    if include_dimensions is None:
        legacy_flags = {
            "potions": include_potions, "companion": include_companion,
            "enc_state": include_encounters, "coop_team": include_coop,
        }
        include_dimensions = {
            d.key: (True if legacy_flags.get(d.key) is None
                    else bool(legacy_flags.get(d.key)))
            for d in cs.DIMENSIONS
        }
    for d in cs.DIMENSIONS:
        if not include_dimensions.get(d.key, True):
            skip.add(d.key)
...
def _checkpoint_with_shape(run, included):
    """按形状矩阵的一行（{维度key: bool}）取校验点。"""
    return state_checkpoint(run, include_dimensions=included)
...
def _create_ckpt_candidates(seed, create_payload):
    ...
    形状候选（哪些跨章维度参与哈希）由 chapterstate 注册表的 shape_matrix 统一
    枚举——结构维度增加时候选组合自动扩展，无需在此手写多重循环。

    返回 (正确初态, create帧比对ckpt, 错误ckpt或None, 命中形状{dim:present})。
    """
    ...
    # 注册表形状矩阵：完整形状优先，再按规则引入顺序的逆序逐层剥字段。
    # 每种形状同时对正确初态与旧版错误初态取哈希（受影响章的 create ckpt
    # 记录的是错误初态）。
    shapes = []
    for name, included in cs.shape_matrix():
        buggy_hash = _checkpoint_with_shape(buggy, included) if buggy is not None else None
        shapes.append((name, included, buggy_hash,
                       _checkpoint_with_shape(sim, included)))
    recorded = create_payload.get("ckpt") if not create_payload.get("_corrupt") else None
    _, matched_included, matched_buggy_ckpt, _ = shapes[0]  # 默认 full
    if recorded is not None:
        for shape in shapes:
            if recorded in (shape[2], shape[3]):
                _, matched_included, matched_buggy_ckpt, _ = shape
                break
    ...
    fixed_ckpt = recorded if recorded is not None else _checkpoint_with_shape(
        sim, {d.key: True for d in cs.DIMENSIONS})
    return sim, fixed_ckpt, matched_buggy_ckpt, dict(matched_included)
...
def replay(run_id):
    ...
    # 命中的旧 create 形状（哪些跨章维度缺席）交给统一的 LegacyWindows 跟踪。
    sim, initial_ckpt, buggy_ckpt, create_included = _create_ckpt_candidates(
        seed, create_payload)
    recorded_initial = (create_payload.get("ckpt")
                        if not create_payload.get("_corrupt") else None)
    windows = cs.LegacyWindows(create_included)
    # 2.7.0 格挡/援护顺序修复等纯规则窗口（无存档结构变更）统一从 create 起点
    # 进入旧时序：首个 baseline+ 动作之前按旧规则推演（见 windows）。
    windows.enter_old_window(sim)
    ...
    for ev in events:
        ...
        # （无日志，首个新版动作之前）与 /act 迁移（本步动作推演之前）都已让新
        # 规则生效。若在动作之后才切换，首个新版动作若是「进入新商店」，回放仍
        # 按旧规则生成库存（缺药水/伙伴货架），后续升级后的购买动作将无法重放，
        # 在线与回放的库存/背包随之分叉。各结构维度与纯规则窗口由注册表统一驱动。
        any_crossing = windows.any_crossing(a, ver)
        ...
        at_fix_point = (legacy_chapter and not post_fix and a != "create"
                        and (migrated_step or (ver and not _ver_lt(ver, RULES_VERSION))
                             or any_crossing))
        if any_crossing:
            windows.apply_crossings(sim, a, ver)
        if at_fix_point:
            post_fix = True
            # 2.4.0 修复点会一并越过尚未触发的伙伴/药水/奇遇/协作/格挡窗口
            # （首个当前版本事件不可能早于它们）：规则先行切换、字段补齐。
            windows.close_all(sim)
        ...
        create_of_legacy = (legacy_chapter and a == "create"
                            and not post_fix)
        # 仍处于任一跨章维度旧结构区间的步骤（字段在录制哈希中缺席）。跨越结构
        # 迁移的本步不再属于旧区间——在线迁移先于本步动作，逐位可比。
        pre_dimension_step = windows.legacy_step(a)
        # 仍处于纯规则旧时序区间的步骤（如 2.7.0 格挡修复点之前）：同一动作在
        # 旧时序下的落库状态与新推演不同（格挡/承伤/伙伴生命），按 legacy 呈现并
        # 跳过哈希比对；动作仍经 legacy_block 旧时序逐位重演。纯规则修复、无结构
        # 变化，因此 create 帧不豁免（create 不含战斗，旧/新初态逐位相同）。
        pre_rule_step = windows.rule_pre_step(a)
        ...
        skip_ckpt = (corrupt_row or migrated_step or pre_repair
                     or create_of_legacy or pre_dimension_step or pre_rule_step)
        if (is_legacy or migrated_step or pre_repair or create_of_legacy
                or pre_dimension_step or pre_rule_step):
            legacy_steps += 1
        if pre_repair:
            repaired_steps += 1
        ...
        if corrupt_row:
            ...
            try:
                ...
                # 已在 _after_battle_step 按旧规则不生成）。
                # 注意：这里以【本步录制版本】为准，而不是字段迁移标记——2.4.0
                # 单局的 create 形状已含 potions 键（结构早在 2.5.0 之前的某版
                # 补入），但 2.4.0 动作生成的商店确实没有药水货架，必须照剥。
                if sim.get("shop") and windows.step_pre_version("companion", ver):
                    sim["shop"].pop("companions", None)
                if sim.get("shop") and windows.step_pre_version("potions", ver):
        ...
        # 纯规则瞬态键在 _CKPT_SKIP_KEYS 中，不影响哈希取值。
        actual = initial_ckpt if a == "create" else state_checkpoint(
            sim, include_dimensions=windows.checkpoint_includes())
        if error:
            status = "error"
        elif not recorded:
